...
if __name__ == '__main__':
    ...
    with open(path) as f:
        ...
        for i in range(0, len(s) - 1, 4):
            ...
            _B.append([_Bx, _By, _Bz])
            _Levels.append([_level1, _level2])

        # foreachで回せばデータ解析可能（Bx, By, Bz）x 250922 個
        _B = np.array(_B)
        _Levels = np.array(_Levels)

    # ↑データの読み込み↑ （20221218_TestFile_Read.py）
    # ↓データ処理↓　（20221214_Check_isMagicPoint.py）

    # 欲しいデータ　遷移周波数，Gradient:S1，Curvature:S2，遷移強度:f
    MagneticField: {np.ndarray: (250922, 3, 3)} = []
    Frequency: {np.ndarray: (250922,)} = []
    Levels: {np.ndarray: (250922, 2)} = []
    Gradient: {np.ndarray: (250922, 3)} = []
    Gradient_INT: {np.ndarray: (250922,)} = []
    Curvature: {np.ndarray: (250922, 3, 3)} = []
    Curvature_INT: {np.ndarray: (250922,)} = []
    f: {np.ndarray: (250922,)} = []

    # 全磁場（250922点）に対して計算を行うのでforループ
    for _num in range(len(_B)):
        dim = 3
        level1 = _Levels[_num, 0]
        level2 = _Levels[_num, 1]

        B: {np.ndarray: (3,)} = np.transpose([_B[_num, 0], _B[_num, 1], _B[_num, 2]])
        ...
        if np.any(V >= 1e-02):
            ...
            Frequency.append(energy[:, level2] - energy[:, level1])
            Levels.append(np.array([level1, level2]))
            Gradient.append(V)
            Gradient_INT.append(np.linalg.norm(V))
            Curvature.append(C)
            Curvature_INT.append(np.max(C))
            f.append(transitionStrength_x[:, level1, level2])
            print(len(f))
    ...
    Curvature_INT = np.array(Curvature_INT)
    f = np.array(f)

    # Sort処理
    sorted_idx = np.argsort(Curvature_INT)
    #
    Frequency = Frequency[sorted_idx]
    Levels = Levels[sorted_idx]
    Gradient = Gradient[sorted_idx]
    Gradient_INT = Gradient_INT[sorted_idx]
    Curvature = Curvature[sorted_idx]
    Curvature_INT = Curvature_INT[sorted_idx]
    f = f[sorted_idx]
    ...
    print(Curvature_INT[0])
    print(Curvature_INT[len(Curvature_INT) - 1])

    np.savez('test.npz', MagneticField=MagneticField, Frequency=Frequency, Levels=Levels,
             Gradient=Gradient, Gradient_INT=Gradient_INT, Curvature=Curvature, Curvature_INT=Curvature_INT, f=f)
