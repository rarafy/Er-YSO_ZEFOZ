...
if __name__ == '__main__':
    ...
    for bx in range(-100, 105, 10):
        for by in range(-100, 105, 10):
            for bz in range(-100, 105, 10):
                for p in range(0, 16):
                        ...
                        try:
                            for i in range(0, 20):
                                ...
                                for _p in range(0, 16):
                                    ...
                                    for _q in range(0, 16):
                                        if _p == _q:
                                            continue
                                        else:
                                            _tempTempCurvature_xx += (perturbationMatrix_x[_p, _q] * perturbationMatrix_x[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_xy += (perturbationMatrix_x[_p, _q] * perturbationMatrix_y[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_xz += (perturbationMatrix_x[_p, _q] * perturbationMatrix_z[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_yx += (perturbationMatrix_y[_p, _q] * perturbationMatrix_x[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_yy += (perturbationMatrix_y[_p, _q] * perturbationMatrix_y[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_yz += (perturbationMatrix_y[_p, _q] * perturbationMatrix_z[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_zx += (perturbationMatrix_z[_p, _q] * perturbationMatrix_x[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_zy += (perturbationMatrix_z[_p, _q] * perturbationMatrix_y[_q, _p]) / (val[_p] - val[_q]) * 2
                                            _tempTempCurvature_zz += (perturbationMatrix_z[_p, _q] * perturbationMatrix_z[_q, _p]) / (val[_p] - val[_q]) * 2
                                ...
                                C = np.array([[energy_curvature_xx[0, q] - energy_curvature_xx[0, p], energy_curvature_xy[0, q] - energy_curvature_xy[0, p],
                                               energy_curvature_xz[0, q] - energy_curvature_xz[0, p]],
                                              [energy_curvature_yx[0, q] - energy_curvature_yx[0, p], energy_curvature_yy[0, q] - energy_curvature_yy[0, p],
                                               energy_curvature_yz[0, q] - energy_curvature_yz[0, p]],
                                              [energy_curvature_zx[0, q] - energy_curvature_zx[0, p], energy_curvature_zy[0, q] - energy_curvature_zy[0, p],
                                               energy_curvature_zz[0, q] - energy_curvature_zz[0, p]]])
                                V = np.transpose([energy_gradient_x[0, q] - energy_gradient_x[0, p], energy_gradient_y[0, q] - energy_gradient_y[0, p],
                                                  energy_gradient_z[0, q] - energy_gradient_z[0, p]])
                                C = np.real(C)
                                B = B - 0.5 * np.dot(np.linalg.inv(C), V)

                                # print(str(C[0]) + ", " + str(C[1]) + ", ")  # + str(C[2]))
                                # print(str(V[0]) + ", " + str(V[1]) + ", " + str(V[2]))
                            print(str(p) + "-" + str(q) + ": " + str(B[0]) + ", " + str(B[1]) + ", " + str(B[2]))
                        except:
                            print(str(p) + "-" + str(q) + ": " + "except")
                            continue
